...
class IndyCredFormatHandler(V20CredFormatHandler):
    ...
    async def create_offer(
        ...
        cred_def_id = await self._match_sent_cred_def_id(
            cred_proposal_message.filter(self.format)
        )
    ...
    async def create_request(
        ...
        if not cred_ex_record.cred_offer:
            raise V20CredFormatError(
                "Indy format requires credential offer from issuer to create request"
            )

        cred_offer = V20CredOffer.deserialize(cred_ex_record.cred_offer).offer(
            self.format
        )
    ...
    async def store_credential(self, cred_ex_record: V20CredExRecord, cred_id: str):
        cred = V20CredIssue.deserialize(cred_ex_record.cred_issue).cred(self.format)
